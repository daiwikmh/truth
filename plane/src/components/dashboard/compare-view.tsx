...
export function CompareView({ evaluations }: CompareViewProps) {
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.3, ease: EASE }}
    >
      <span className="text-[9px] tracking-[0.2em] uppercase text-muted-foreground font-mono block">
        PAIRWISE COMPARISON
      </span>
      ...
      <div className="flex flex-wrap gap-4 items-end">
        <div className="space-y-1">
          <label className="text-[9px] tracking-[0.2em] uppercase text-muted-foreground font-mono">
            PROJECT A
          </label>
          <select
            value={idxA}
            onChange={(e) => setIdxA(Number(e.target.value))}
            className="block border border-foreground/20 rounded-lg bg-background text-[11px] font-mono px-3 py-2 focus:outline-none focus:border-foreground"
          >
        ...
        <span className="text-[11px] font-mono text-muted-foreground pb-2">
          vs
        </span>
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            className="block border border-foreground/20 rounded-lg bg-background text-[11px] font-mono px-3 py-2 focus:outline-none focus:border-foreground w-48"
          />
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="border border-foreground/20 rounded-lg px-5 py-2 text-[10px] tracking-[0.2em] uppercase font-mono hover:bg-foreground hover:text-background transition-colors disabled:opacity-40"
        >
      ...
      {result && (
        ...
        >
          ...
          <div className="border border-foreground/15 rounded-xl p-5">
            <span className="text-[9px] tracking-[0.2em] uppercase text-muted-foreground font-mono block mb-2">
              WINNER
            </span>
            ...
            <p className="text-[11px] font-mono text-muted-foreground/80 mt-3 leading-relaxed">
              {result.reasoning}
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              ...
              <div key={s.label} className="border border-foreground/10 rounded-xl p-3">
                ...
                <span
                  className="text-[13px] font-mono font-bold mt-1 block"
          ...
          {result.dimensions?.length > 0 && (
            <div className="border border-foreground/10 rounded-xl">
              <div className="border-b-2 border-foreground/10 px-4 py-2 grid grid-cols-[1fr_60px_60px_2fr] gap-2">
                ...
                <span className="text-[9px] tracking-[0.2em] uppercase text-muted-foreground font-mono text-center">
                  A
                </span>
                <span className="text-[9px] tracking-[0.2em] uppercase text-muted-foreground font-mono text-center">
                  B
              ...
              {result.dimensions.map((dim, i) => (
                ...
                >
                  ...
                  <span
                    className="text-[11px] font-mono text-center font-bold"
